...
export class Window {
  ...
  createWindows(options: object) {
    ...
    if (app.isPackaged) {
      ...
      winURL = args.route
        ? `http://${import.meta.env['VITE_DEV_SERVER_HOST']}:${import.meta.env['VITE_DEV_SERVER_PORT']}${args.route}?winId=${args.id}`
        : `http://${import.meta.env['VITE_DEV_SERVER_HOST']}:${import.meta.env['VITE_DEV_SERVER_PORT']}${import.meta.env['VITE_DEV_SERVER_HOMEURL']}?winId=${args.id}`
    }
