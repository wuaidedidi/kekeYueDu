...
// 初始化版本表
const initVersionTable = async (db) => {
    await db.exec(`
...
const createVersionTable = async () => {
    ...
    // 检查版本表是否存在
    const result = await db.get("SELECT name FROM sqlite_master WHERE type='table' AND name='chapter_versions'");
    if (!result) {
        // 表不存在，初始化
...
// 创建新版本
const createVersion = async (chapterId, contentHtml, source = 'manual', label, isSnapshot = false, authorId = 1) => {
    const db = await connectDB();
    // 获取当前章节的最新版本号
    const latestVersion = await db.get('SELECT MAX(version_seq) as max_seq FROM chapter_versions WHERE chapter_id = ?', [chapterId]);
    ...
    const result = await db.run(`INSERT INTO chapter_versions (
...
// 获取章节版本列表
const getChapterVersions = async (chapterId, page = 1, limit = 20, source) => {
    const db = await connectDB();
    ...
    // 获取总数量
    const totalResult = await db.get(`SELECT COUNT(*) as total FROM chapter_versions ${whereClause}`, params);
    const total = totalResult.total;
    // 获取版本列表
    const versions = await db.all(`SELECT
...
// 切换版本置顶状态
const togglePinVersion = async (chapterId, versionId, pinned) => {
    const db = await connectDB();
    await db.run('UPDATE chapter_versions SET is_pinned = ? WHERE id = ? AND chapter_id = ?', [pinned ? 1 : 0, versionId, chapterId]);
    const version = await db.get('SELECT * FROM chapter_versions WHERE id = ? AND chapter_id = ?', [versionId, chapterId]);
    await db.close();
    if (!version) {
...
// 删除版本
const deleteVersion = async (chapterId, versionId) => {
    const db = await connectDB();
    const result = await db.run('DELETE FROM chapter_versions WHERE id = ? AND chapter_id = ?', [versionId, chapterId]);
    await db.close();
    if (result.changes === 0) {
...
// 获取指定版本
const getVersion = async (chapterId, versionId) => {
    const db = await connectDB();
    const version = await db.get('SELECT * FROM chapter_versions WHERE id = ? AND chapter_id = ?', [versionId, chapterId]);
    await db.close();
    if (!version) {
...
// 回退到指定版本
const revertToVersion = async (chapterId, toVersionId, authorId = 1) => {
    const db = await connectDB();
    // 开始事务
    await db.run('BEGIN TRANSACTION');
    try {
        // 获取目标版本内容
        const targetVersion = await db.get('SELECT * FROM chapter_versions WHERE id = ? AND chapter_id = ?', [toVersionId, chapterId]);
        if (!targetVersion) {
            throw new Error('目标版本不存在');
        }
        // 更新章节内容
        await db.run('UPDATE Chapters SET content = ? WHERE id = ?', [targetVersion.content_html, chapterId]);
        // 创建回退版本
        const latestVersionResult = await db.get('SELECT MAX(version_seq) as max_seq FROM chapter_versions WHERE chapter_id = ?', [chapterId]);
...
export { createVersionTable, createVersion, getChapterVersions, togglePinVersion, deleteVersion, getVersion, revertToVersion, };
