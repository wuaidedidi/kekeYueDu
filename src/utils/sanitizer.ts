...
export class Sanitizer {
  ...
  static sanitizeForExport(dirty: string): string {
    return this.sanitize(dirty, {
      ...
      ALLOWED_ATTR: [
        ...this.defaultConfig.ALLOWED_ATTR,
        'charset', 'name', 'content', 'rel', 'type', 'media'
      ],
      // 允许内联样式用于导出
      ALLOWED_STYLES: [
        'color', 'background-color', 'font-size', 'font-weight',
        'text-align', 'margin', 'padding', 'border', 'line-height',
        'width', 'height', 'display', 'position', 'text-decoration'
      ]
    })
  }
  ...
  static sanitizeMarkdown(html: string): string {
    return this.sanitize(html, {
      ALLOWED_TAGS: [
        ...this.defaultConfig.ALLOWED_TAGS,
        'mark', 'kbd', 'samp', 'var', 'abbr', 'cite', 'dfn'
      ],
      ALLOWED_ATTR: [
        ...this.defaultConfig.ALLOWED_ATTR,
        'data-footnote-ref', 'data-footnote-id'
      ]
    })
  }
   ...
   * 添加自定义钩子函数
   */
  static addHook(name: 'uponSanitizeAttribute' | 'uponSanitizeElement', hook: DOMPurify.Hook): void {
   ...
   * 移除钩子函数
   */
  static removeHook(name: 'uponSanitizeAttribute' | 'uponSanitizeElement', hook: DOMPurify.Hook): void {
  ...
  static initialize(): void {
    ...
    this.addHook('uponSanitizeAttribute', (node, data, config) => {
      ...
      if (data.attrName === 'style') {
        // 可以在这里添加更严格的样式检查
        const dangerousStyles = ['position: fixed', 'position: absolute', 'z-index']
    ...
    this.addHook('uponSanitizeElement', (node, data, config) => {
      ...
      if (node.nodeName === 'SCRIPT' || node.nodeName === 'STYLE') {
        ...
        if (node.nodeName === 'STYLE' && node.textContent) {
          ...
          const hasDangerousCSS = dangerousCSS.some(css => node.textContent!.includes(css))
          if (hasDangerousCSS) {
            // 过滤危险样式
...
// 导出便捷方法
export const sanitizeHTML = (dirty: string, config?: Partial<DOMPurify.Config>) => Sanitizer.sanitize(dirty, config)
export const sanitizeText = (text: string) => Sanitizer.sanitizeText(text)
export const sanitizeForPreview = (dirty: string) => Sanitizer.sanitizeForPreview(dirty)
export const sanitizeForExport = (dirty: string) => Sanitizer.sanitizeForExport(dirty)
export const sanitizeMarkdown = (html: string) => Sanitizer.sanitizeMarkdown(html)

// 统一的HTML安全渲染方法
export const sanitizeHtml = (dirty: string, context: 'preview' | 'export' | 'general' = 'general'): string => {
  if (!dirty || typeof dirty !== 'string') {
    return ''
  }
...
// 验证HTML安全性
export const validateHtmlSafety = (html: string): { isSafe: boolean; warnings: string[] } => {
  ...
  // 检查危险属性
  const dangerousAttrs = ['onclick', 'onload', 'onerror', 'onmouseover', 'javascript:', 'data:text/html']
