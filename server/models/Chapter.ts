...
const initChapterTable = async (db: Database) => {
  ...
  for (const chapter of chapters) {
    await db.run(
      'INSERT INTO Chapters (volume_id, title, content, `order`) VALUES (?, ?, ?, ?)',
      [chapter.volume_id, chapter.title, chapter.content, chapter.order]
    )
  }
}
...
const getChapterContent = async (id: number) => {
  try {
    const db = await connectDB()
