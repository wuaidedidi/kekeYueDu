...
export interface ShopProduct {
  ...
  subtitle?: string
  description?: string
  type: string // 权益类型：vip, card, coupon, skin等
  price: number
  charge_mode: string // 计费模式：points, times, duration
  duration_days?: number // 有效期天数
  times?: number // 使用次数
  activation_required?: boolean // 是否需要激活
  icon_url: string
  status: 'active' | 'inactive' | 'sold_out'
  stock?: number // 库存，-1表示无限制
  created_at?: string
  updated_at?: string
}
...
export class ShopModel {
  ...
  async getProducts(filters?: {
    ...
    return new Promise((resolve, reject) => {
      ...
      this.db.all(query, params, (err, rows) => {
        ...
        // 获取总数
        this.db.get(countQuery, countParams, (countErr, countRow: any) => {
          if (countErr) {
            reject(countErr)
            return
          }
  ...
  async getUserRights(userId: number): Promise<UserRight[]> {
    return new Promise((resolve, reject) => {
      this.db.all(
        'SELECT * FROM user_rights WHERE user_id = ? ORDER BY created_at DESC',
        [userId],
        (err, rows) => {
          if (err) {
            reject(err)
          } else {
            resolve(rows as UserRight[])
          }
        }
      )
    })
  ...
  async getUserOrders(userId: number, page: number = 1, pageSize: number = 20): Promise<{
    ...
    return new Promise((resolve, reject) => {
      ...
      this.db.all(query, [userId, pageSize, offset], (err, rows) => {
        ...
        this.db.get(countQuery, [userId], (countErr, countRow: any) => {
          if (countErr) {
            reject(countErr)
            return
          }
